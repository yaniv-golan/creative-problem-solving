...
Everything above is reproducible from [`evals/`](evals/README.md) — definitions, per-assertion
grading, and the full transcript of every run in both configurations, **including the rounds
where the skill lost**. That is unusual for a skill and it is the point.
